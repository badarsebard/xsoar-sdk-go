/*
Cortex XSOAR API

This is the public REST API to integrate with the Cortex XSOAR server. HTTP request can be sent using any HTTP-client.  For an example dedicated client take a look at: https://github.com/demisto/demisto-py.  Requests must include API-key that can be generated in the Cortex XSOAR web client under 'Settings' -> 'Integrations' -> 'API keys'   Optimistic Locking and Versioning\\:  When using Cortex XSOAR REST API, you will need to make sure to work on the latest version of the item (incident, entry, etc.), otherwise, you will get a DB version error (which not allow you to override a newer item). In addition, you can pass 'version\\: -1' to force data override (make sure that other users data might be lost).  Assume that Alice and Bob both read the same data from Cortex XSOAR server, then they both changed the data, and then both tried to write the new versions back to the server. Whose changes should be saved? Alice’s? Bob’s? To solve this, each data item in Cortex XSOAR has a numeric incremental version. If Alice saved an item with version 4 and Bob trying to save the same item with version 3, Cortex XSOAR will rollback Bob request and returns a DB version conflict error. Bob will need to get the latest item and work on it so Alice work will not get lost.  Example request using 'curl'\\:  ``` curl 'https://hostname:443/incidents/search' -H 'content-type: application/json' -H 'accept: application/json' -H 'Authorization: <API Key goes here>' --data-binary '{\"filter\":{\"query\":\"-status:closed -category:job\",\"period\":{\"by\":\"day\",\"fromValue\":7}}}' --compressed ```

API version: 2.0.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// AutomationScriptFilterWrapper struct for AutomationScriptFilterWrapper
type AutomationScriptFilterWrapper struct {
	Filter       *GenericStringFilter `json:"filter,omitempty"`
	SavePassword *bool                `json:"savePassword,omitempty"`
	Script       *AutomationScript    `json:"script,omitempty"`
}

// NewAutomationScriptFilterWrapper instantiates a new AutomationScriptFilterWrapper object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewAutomationScriptFilterWrapper() *AutomationScriptFilterWrapper {
	this := AutomationScriptFilterWrapper{}
	return &this
}

// NewAutomationScriptFilterWrapperWithDefaults instantiates a new AutomationScriptFilterWrapper object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewAutomationScriptFilterWrapperWithDefaults() *AutomationScriptFilterWrapper {
	this := AutomationScriptFilterWrapper{}
	return &this
}

// GetFilter returns the Filter field value if set, zero value otherwise.
func (o *AutomationScriptFilterWrapper) GetFilter() GenericStringFilter {
	if o == nil || o.Filter == nil {
		var ret GenericStringFilter
		return ret
	}
	return *o.Filter
}

// GetFilterOk returns a tuple with the Filter field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AutomationScriptFilterWrapper) GetFilterOk() (*GenericStringFilter, bool) {
	if o == nil || o.Filter == nil {
		return nil, false
	}
	return o.Filter, true
}

// HasFilter returns a boolean if a field has been set.
func (o *AutomationScriptFilterWrapper) HasFilter() bool {
	if o != nil && o.Filter != nil {
		return true
	}

	return false
}

// SetFilter gets a reference to the given GenericStringFilter and assigns it to the Filter field.
func (o *AutomationScriptFilterWrapper) SetFilter(v GenericStringFilter) {
	o.Filter = &v
}

// GetSavePassword returns the SavePassword field value if set, zero value otherwise.
func (o *AutomationScriptFilterWrapper) GetSavePassword() bool {
	if o == nil || o.SavePassword == nil {
		var ret bool
		return ret
	}
	return *o.SavePassword
}

// GetSavePasswordOk returns a tuple with the SavePassword field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AutomationScriptFilterWrapper) GetSavePasswordOk() (*bool, bool) {
	if o == nil || o.SavePassword == nil {
		return nil, false
	}
	return o.SavePassword, true
}

// HasSavePassword returns a boolean if a field has been set.
func (o *AutomationScriptFilterWrapper) HasSavePassword() bool {
	if o != nil && o.SavePassword != nil {
		return true
	}

	return false
}

// SetSavePassword gets a reference to the given bool and assigns it to the SavePassword field.
func (o *AutomationScriptFilterWrapper) SetSavePassword(v bool) {
	o.SavePassword = &v
}

// GetScript returns the Script field value if set, zero value otherwise.
func (o *AutomationScriptFilterWrapper) GetScript() AutomationScript {
	if o == nil || o.Script == nil {
		var ret AutomationScript
		return ret
	}
	return *o.Script
}

// GetScriptOk returns a tuple with the Script field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AutomationScriptFilterWrapper) GetScriptOk() (*AutomationScript, bool) {
	if o == nil || o.Script == nil {
		return nil, false
	}
	return o.Script, true
}

// HasScript returns a boolean if a field has been set.
func (o *AutomationScriptFilterWrapper) HasScript() bool {
	if o != nil && o.Script != nil {
		return true
	}

	return false
}

// SetScript gets a reference to the given AutomationScript and assigns it to the Script field.
func (o *AutomationScriptFilterWrapper) SetScript(v AutomationScript) {
	o.Script = &v
}

func (o AutomationScriptFilterWrapper) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if o.Filter != nil {
		toSerialize["filter"] = o.Filter
	}
	if o.SavePassword != nil {
		toSerialize["savePassword"] = o.SavePassword
	}
	if o.Script != nil {
		toSerialize["script"] = o.Script
	}
	return json.Marshal(toSerialize)
}

type NullableAutomationScriptFilterWrapper struct {
	value *AutomationScriptFilterWrapper
	isSet bool
}

func (v NullableAutomationScriptFilterWrapper) Get() *AutomationScriptFilterWrapper {
	return v.value
}

func (v *NullableAutomationScriptFilterWrapper) Set(val *AutomationScriptFilterWrapper) {
	v.value = val
	v.isSet = true
}

func (v NullableAutomationScriptFilterWrapper) IsSet() bool {
	return v.isSet
}

func (v *NullableAutomationScriptFilterWrapper) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableAutomationScriptFilterWrapper(val *AutomationScriptFilterWrapper) *NullableAutomationScriptFilterWrapper {
	return &NullableAutomationScriptFilterWrapper{value: val, isSet: true}
}

func (v NullableAutomationScriptFilterWrapper) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableAutomationScriptFilterWrapper) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
